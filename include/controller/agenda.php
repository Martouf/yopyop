<?php
/*******************************************************************************************
 * Nom du fichier		: agenda.php
 * Date					: 26 juin 2009 
 * Auteur				: Mathieu Despont
 * Adresse E-mail		: mathieu@ecodev.ch
 * But de ce fichier	: Afficher les calendriers en lecture seule. C'est le clone de calendrier.php mais sans les fonctions d'édition
 *******************************************************************************************
...
/*
 *  Attention, il faut encore mettre en place la gestion des permissions !!! (le couplage avec la variable de session typo3 est fait)
 *
 */
...
// détermine le type de vue: jour, mois, semaine, année, liste... par défaut mois.
$typeDeVue = "mois";
...
if ($action=='get') {
		...
		if ($outputFormat=='html') {
			...
			if ($typeDeVue=="semaine") {
				...
				$editable = false;  // cette variable sert aux templates à savoir si il faut afficher les liens qui appellent les fonctions javascript d'édition.
			...
			/*********** carte ********************/
			}elseif ($typeDeVue=="carte") {
				
				// l'affichage des points sur la carte est fait par ajax, il n'y a donc pas ici besoin d'aller chercher des événements.
				// Cette opération est faite par un javascript qui va chercher des donnée au format json
				// ici on doit juste mettre en place la machinerie de base de la carte
			...
			// TODO
			// prendre aussi les événements qui sont aujourd'hui mais qui ont commencé dans le passé
			...
			foreach ($listeEvenement as $key => $idEvenement) {
				...
				// reprise des données d'un calendrier en fonction de son id
		...
		if ($typeDeVue=='carte') {
			// pour charger une machinerie cartographique
		 	$additionalHeader .= "
		...
		// si le calendrier est éditable, ajoute les script nécessaire à son édition
		if ($editable) {
			$additionalHeader .= "<script type=\"text/javascript\" src=\"http://".$_SERVER['SERVER_NAME']."/utile/js/calendrier_edit_".$typeDeVue.".js\"></script>";
		}else{
			$additionalHeader .= "<script type=\"text/javascript\" src=\"http://".$_SERVER['SERVER_NAME']."/utile/js/calendrier_lecture_".$typeDeVue.".js\"></script>";	
		}
		...
		if ($outputFormat=='ics') {
			...
			// suite à une demande d'exportation on place l'état de l'événement à Exporté.
			$listeEvenementsExportes = array_keys($evenements);
			foreach ($listeEvenementsExportes as $key => $idEvenementExporte) {
				$evenementManager->setEvenementExported($idEvenementExporte);
			}
			
			// exportation des données en xml pour in design			
			...
			$smarty->display("calendrier_".LANG."_".$outputFormat.".tpl");
		}else{
			...
			if ($theme=="no") {
				$smarty->display("agenda_".$typeDeVue."_".LANG."_html.tpl"); // affichage de la ressource brute dans la langue demandée et le format demandé
			}else{
				// affiche la ressource inclue dans le template du thème index.tpl
				$smarty->assign('contenu',"agenda_".$typeDeVue."_".LANG."_html.tpl"); // va chercher le template de la ressource demandée, dans la langue demandée et dans le format de sortie demandé.
