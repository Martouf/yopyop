...
if ($action=='get') {
	...
	// un groupe de ressources
		...
		if (empty($tags)) {
			$tousLieux = $lieuManager->getLieux();
			...
			foreach ($tousLieux as $key => $aLieu) {
				$lieu = $aLieu;
				
				$lieu['nomSimplifie'] = simplifieNom($aLieu['nom']);				
				$lieux[$aLieu['id_lieu']] = $lieu;		
			}
		}else{
			...
			foreach ($taggedElements as $key => $idLieu) {
				...
				$lieux[$idLieu]['nomSimplifie'] = simplifieNom($lieux[$idLieu]['nom']);
			}
		} // if $tags
...
///////////////
	...
	// passe les données du lieu à l'affichage
	$smarty->assign('lieu',$lieu);
