	<div id="utilisateur">
		...
		<div id="colonneDetailUtilisateur" <!--{if $utilisateurConnu}--> ondblclick="document.location='<!--{$personne.id_personne}-->-<!--{$personne.nomSimplifie}-->.html?modify';"<!--{/if}--> >
			...
			<div id="barreEtatPersonne">
				<!--{if $utilisateurConnu}--><a href="/utilisateur/<!--{$personne.id_personne}-->-<!--{$personne.nomSimplifie}-->.html?modify" title="modifier les données de la personne"><img src="/utile/img/vcard_edit.png" alt="editer"/>&nbsp;modifier</a><!--{/if}-->
			</div>
		</div>
		<hr />
	</div>
